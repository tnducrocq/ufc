...
public class ElasticDismissLayout extends FrameLayout {
    ...
    public ElasticDismissLayout(Context context) {
        this(context, null, 0, 0);
    }

    public ElasticDismissLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0, 0);
    }

    public ElasticDismissLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public ElasticDismissLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public void onStopNestedScroll(View child) {
        if (Math.abs(totalDrag) >= dragDismissDistance) {
            ...
            animate().translationY(0f).scaleX(1f).scaleY(1f).setDuration(200L).setInterpolator(PresentationUtils.getFastOutSlowInInterpolator(getContext())).setListener(null).start();
            totalDrag = 0;
    ...
    public static class SystemChromeFader extends ElasticDragDismissCallback {
        ...
        public SystemChromeFader(Activity activity) {
            ...
            fadeNavBar = PresentationUtils.isNavBarOnBottom(activity);
        }
        ...
        public void onDrag(float elasticOffset, float elasticOffsetPixels, float rawOffset, float rawOffsetPixels) {
            if (elasticOffsetPixels > 0) {
                ...
                activity.getWindow().setStatusBarColor(PresentationUtils.modifyAlpha(activity.getWindow().getStatusBarColor(), (int) ((1f - rawOffset) * statusBarAlpha)));
            } else if (elasticOffsetPixels == 0) {
                // reset
                activity.getWindow().setStatusBarColor(PresentationUtils.modifyAlpha(activity.getWindow().getStatusBarColor(), statusBarAlpha));
                activity.getWindow().setNavigationBarColor(PresentationUtils.modifyAlpha(activity.getWindow().getNavigationBarColor(), navBarAlpha));
            } else if (fadeNavBar) {
                // dragging upward, fade the navigation bar in proportion
                activity.getWindow().setNavigationBarColor(PresentationUtils.modifyAlpha(activity.getWindow().getNavigationBarColor(), (int) ((1f - rawOffset) * navBarAlpha)));
            }
        }
